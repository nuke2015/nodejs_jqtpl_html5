var SpinningWheel = {cellHeight: 86, friction: .003, slotData: [], exectimeout: null, touchY: 0, handleEvent: function (e) {
    ...
    console.info("NND 执行不？" + oldresults.keys);
    var self = this;
    var index = this.getChangedSlotIndex(oldresults.keys, results.keys);
    console.info("gggggggggg---index---------()" + index);
    console.info("NND搞死了" + results.keys);
    if (index > -1) {
        console.info(this.slotData[index].scrollDone);
        console.info("gmgmgmgmggmgmgggggggggg---index---------" + index);
        if (typeof self.slotData[index].scrollDone == "function") {
            console.info("gggggggggg---index----scrollDone-----");
            clearTimeout(self.exectimeout);
            var currSlotKey = self.getSlotKey(index);
            console.info("gggggggggg---index----scrollDone----currSlotKey-" + currSlotKey);
            self.exectimeout = setTimeout(function () {
                console.info("will exec the scrollDone");
    ...
    style = style.join(" ");
    console.info("hhhh--" + typeof scrollDone);
    ...
    console.info("this.slotEl[slot].slotYPosition = " + this.slotEl[slot].slotYPosition)
   ...
   // alert("tapdown");
    ...
    if (e.currentTarget.id == "sw-cancel") {
        ...
        console.info("tapUptapUptapUp = " + this.checkBeforUp());
